...
class MaterializedViewLeftJoinRuleSuite extends RewriteSuite {
  ...
  test("mv_left_join") {
    spark.sql(
      """
        |DROP MATERIALIZED VIEW IF EXISTS mv_left_join;
        |""".stripMargin
    )
    spark.sql(
      """
        |CREATE MATERIALIZED VIEW IF NOT EXISTS mv_left_join
        |AS
        |SELECT e.*,d.deptname
  ...
  test("mv_left_join_1") {
    ...
    val sql =
      """
        ...
        |ON e.deptno=d.deptno where e.deptno >= 2;
        |""".stripMargin
    comparePlansAndRows(sql, "default", "mv_left_join", noData = false)
  }
  ...
  test("mv_left_join_cannot_rewrite") {
    val sql =
      """
        |SELECT e1.*,d.deptname,e2.*
        |FROM emps e1 LEFT JOIN depts d ON e1.deptno=d.deptno JOIN emps e2
