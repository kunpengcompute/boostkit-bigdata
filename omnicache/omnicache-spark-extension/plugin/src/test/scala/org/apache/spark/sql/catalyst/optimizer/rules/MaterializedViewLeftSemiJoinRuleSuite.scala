...
class MaterializedViewLeftSemiJoinRuleSuite extends RewriteSuite {

  test("mv_left_semi_join") {
    spark.sql(
      """
        |DROP MATERIALIZED VIEW IF EXISTS mv_left_semi_join;
        |""".stripMargin
    )
    spark.sql(
      """
        |CREATE MATERIALIZED VIEW IF NOT EXISTS mv_left_semi_join
        |AS
        |SELECT e.*
        |FROM emps e SEMI JOIN depts d
        |ON e.deptno=d.deptno where e.deptno >= 2;
        |""".stripMargin
    )
  }

  test("mv_left_semi_join_1") {
    // is same to view
    val sql =
      """
        |SELECT e.*
        ...
        |ON e.deptno=d.deptno where e.deptno >= 2;
        |""".stripMargin
    comparePlansAndRows(sql, "default", "mv_left_semi_join", noData = false)
  }

  test("mv_left_semi_join_2") {
    // view tables is subset of query
    val sql =
      """
        |SELECT e.*, l.locationid
        |FROM emps e SEMI JOIN depts d ON e.deptno=d.deptno JOIN locations l
        |ON e.locationid=l.locationid where e.deptno >= 2;
        |""".stripMargin
    comparePlansAndRows(sql, "default", "mv_left_semi_join", noData = false)
  }

  test("mv_left_semi_join_3") {
    // view tables is subset of query
    val sql =
      """
        |SELECT e.*, l.locationid
  ...
  test("mv_left_semi_join_cannot_rewrite") {
    val sql =
      """
        |SELECT e1.*,e2.*
