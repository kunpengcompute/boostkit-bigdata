...
object ViewMetadata extends RewriteHelper {
  ...
  def setSpark(sparkSession: SparkSession): Unit = {
    ...
    metadataPath = new Path(OmniMVPluginConfig.getConf.metadataPath)
  ...
  def forceLoadFromMetastore(): Unit = this.synchronized {
    ...
    val dbs = RewriteTime.withTimeStat("loadDbs") {
      if (getConf.omniMVDB.nonEmpty) {
        getConf.omniMVDB.split(",").toSeq
      } else {
   ...
   */
  def omniMVFilter(catalog: SessionCatalog,
  ...
  /**
   * load metadata file when mv's db=omniMVDB and mv exists
  ...
  def filterValidMetadata(): Array[FileStatus] = {
    ...
    if (OmniMVPluginConfig.getConf.enableMetadataInitByQuery && initQueryPlan.isDefined) {
  ...
  private def loadMapFromDisk[K: Manifest, V: Manifest](
    ...
    var readLines = OmniMVPluginConfig.getConf.metadataIndexTailLines
  ...
  /**
   * If "spark.sql.omnimv.dbs" specifies databases,
  ...
  def getDBs: Set[String] = {
    if (OmniMVPluginConfig.getConf.omniMVDB.nonEmpty) {
      OmniMVPluginConfig.getConf.omniMVDB
