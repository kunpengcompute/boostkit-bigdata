...
import com.huawei.boostkit.spark.util.lock.{FileLock, OmniMVAtomic}
...
import org.apache.spark.sql.catalyst.optimizer.OmniMVToSparkAdapter._
...
case class OmniMVCreateMvCommand(
  ...
  override def run(sparkSession: SparkSession, child: SparkPlan): Seq[Row] = {
    try {
      ...
      OmniMVAtomic.funcWithSpinLock(fileLock) {
        () =>
...
case class ShowMaterializedViewCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val omniMVFilter: TableIdentifier => Boolean = {
      tableIdentifier =>
    ...
    val showLength = tableIdentifierPattern match {
      case Some(_) =>
        Integer.MAX_VALUE
      case None =>
        OmniMVPluginConfig.getConf.showMVQuerySqlLen
    }
...
case class WashOutMaterializedViewCommand(
  ...
  private def washOutByReserveQuantity(para: Option[Int]): Unit = {
    val reserveQuantity = para.getOrElse(
      OmniMVPluginConfig.getConf.reserveViewQuantityByViewCount)
