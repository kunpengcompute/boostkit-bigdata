...
class OmniMVExtensionSqlParser(spark: SparkSession,
    delegate: ParserInterface) extends ParserInterface with SQLConfHelper with RewriteLogger {

  private lazy val astBuilder = new OmniMVExtensionAstBuilder(spark, delegate)

  override def parsePlan(sqlText: String): LogicalPlan = {
    if (OmniMVPluginConfig.getConf.enableSqlLog) {
  ...
  def parse[T](command: String)(toResult: OmniMVSqlExtensionsParser => T): T = {
    logDebug(s"Parsing command: $command")

    val lexer = new OmniMVSqlExtensionsLexer(
      new UpperCaseCharStream(CharStreams.fromString(command)))
    lexer.removeErrorListeners()
    lexer.addErrorListener(ParseErrorListener)

    val tokenStream = new CommonTokenStream(lexer)
    val parser = new OmniMVSqlExtensionsParser(tokenStream)
