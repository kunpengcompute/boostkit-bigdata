...
import com.huawei.boostkit.spark.exception.OmniMVException
...
class WashOutMVSuite extends WashOutBase {
  ...
  test("wash out mv by reserve quantity.") {
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.reserve.quantity.byViewCnt", "3")
    val reserveQuantity = OmniMVPluginConfig.getConf.reserveViewQuantityByViewCount
  ...
  test("auto wash out") {
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.unused.day", "0")
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.reserve.quantity.byViewCnt", "1")
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.drop.quantity.bySpaceConsumed", "1")
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.automatic.time.interval", "0")
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.automatic.view.quantity", "1")
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.automatic.enable", "true")
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.automatic.checkTime.interval", "0")
    spark.sql(
      f"""
    ...
    spark.sessionState.conf.setConfString(
      "spark.sql.omnimv.washout.automatic.enable", "false")
  }
}
...
class WashOutBase extends AnyFunSuite
  ...
  lazy val spark: SparkSession = SparkSession.builder().master("local")
      .config("spark.sql.extensions", "com.huawei.boostkit.spark.OmniMV")
      .config("hive.exec.dynamic.partition.mode", "nonstrict")
      .config("spark.ui.port", "4050")
      // .config("spark.sql.planChangeLog.level", "WARN")
      .config("spark.sql.omnimv.logLevel", "WARN")
      .config("spark.sql.omnimv.dbs", "default")
      .config("spark.sql.omnimv.metadata.initbyquery.enable", "false")
      .config("hive.in.test", "true")
      .config("spark.sql.omnimv.metadata.path", "./user/omnimv/metadata")
      .config("spark.sql.omnimv.washout.automatic.enable", "false")
      .enableHiveSupport()
      .getOrCreate()
  ...
  def loadData[K: Manifest, V: Manifest](file: Path,
    ...
    try {
      ...
      case _: Throwable =>
        throw OmniMVException("load data failed.")
    }
  }
  ...
  def saveData[K: Manifest, V: Manifest](file: Path,
    ...
    try {
      ...
      case _: Throwable =>
        throw OmniMVException("save data failed.")
    }
  }
}
