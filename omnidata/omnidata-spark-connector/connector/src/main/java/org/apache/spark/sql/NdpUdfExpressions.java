...
public class NdpUdfExpressions {
    ...
    private void checkAttributeReference(Expression childExpression,
        ...
        if ((childExpression instanceof AttributeReference)) {
            ...
            prestoExpressionInfo.setFieldDataType(
                    NdpUtils.transOlkDataType(childExpression.dataType(), (Attribute) childExpression, false, false));
    ...
    private void createNdpSingleParameter(NdpUdfEnum udfEnum,
        ...
        Attribute attributeChild = null;
        boolean isInputDataTypeChild = true;
        if (childExpression instanceof Attribute) {
            attributeChild = (Attribute) childExpression;
            isInputDataTypeChild = false;
        }
        Type childType = NdpUtils.transOlkDataType(childExpression.dataType(), attributeChild, true, isInputDataTypeChild);
        if (childType instanceof CharType) {
            childType = createVarcharType(((CharType) childType).getLength());
        }
        Type returnType = NdpUtils.transOlkDataType(expression.dataType(), null, true, true);
    ...
    private void createHiveSimpleUdf(Expression hiveSimpleUDFExpression,
        ...
        Type returnType = NdpUtils.transOlkDataType(
                hiveSimpleUDFExpression.dataType(), null, false, true);
        List<RowExpression> rowArguments = new ArrayList<>();
        Type strTypeCandidate = returnType;
        Signature signature;
        for (Expression hiveUdf : hiveSimpleUdf) {
            strTypeCandidate = NdpUtils.transOlkDataType(hiveUdf.dataType(), null, false, true);
    ...
    private void createNdpSubstring(Substring expression, PrestoExpressionInfo prestoExpressionInfo,
        ...
        Type strType = NdpUtils.transOlkDataType(expression.str().dataType(), (Attribute) expression.str(), true, false);
        if (strType instanceof CharType) {
            strType = createVarcharType(((CharType) strType).getLength());
        }
        Type lenType = NdpUtils.transOlkDataType(expression.len().dataType(), null, true, true);
        Type posType = NdpUtils.transOlkDataType(expression.pos().dataType(), null, true, true);
        Type returnType = NdpUtils.transOlkDataType(expression.dataType(), null, true, true);
    ...
    private void createNdpReplace(StringReplace expression,
        ...
        Type srcType = NdpUtils.transOlkDataType(expression.srcExpr().dataType(), (Attribute) expression.srcExpr(), true, false);
        if (srcType instanceof CharType) {
            srcType = createVarcharType(((CharType) srcType).getLength());
        }
        Type searchType = NdpUtils.transOlkDataType(expression.searchExpr().dataType(), null, true, true);
        Type replaceType = NdpUtils.transOlkDataType(expression.replaceExpr().dataType(), null, true, true);
        Type returnType = NdpUtils.transOlkDataType(expression.dataType(), null, true, true);
    ...
    private void createNdpInstr(StringInstr expression, PrestoExpressionInfo prestoExpressionInfo,
        ...
        Type substrType = NdpUtils.transOlkDataType(expression.substr().dataType(), null, true, true);
        Type returnType = NdpUtils.transOlkDataType(expression.dataType(), null, true, true);
    ...
    private void createNdpSplit(StringSplit expression, PrestoExpressionInfo prestoExpressionInfo,
        ...
        Type regexType = NdpUtils.transOlkDataType(expression.regex().dataType(), null, true, true);
        Type returnType = NdpUtils.transOlkDataType(expression.dataType(), null,  true, true);
    ...
    private void createNdpSubscript(GetArrayItem expression,
        ...
        Type strType = NdpUtils.transOlkDataType(expression.child().dataType(), (Attribute) expression.child(), true, false);
        if (strType instanceof CharType) {
            strType = createVarcharType(((CharType) strType).getLength());
        }
        Type ordinalType = NdpUtils.transOlkDataType(expression.ordinal().dataType(), null, true, true);
        Type returnType = NdpUtils.transOlkDataType(expression.dataType(), null, true, true);
