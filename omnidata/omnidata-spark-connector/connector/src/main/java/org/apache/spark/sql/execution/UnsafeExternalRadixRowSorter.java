...
public class UnsafeExternalRadixRowSorter{
  ...
  private final  List<UnsafeExternalRadixRowSorter.PrefixComputer> prefixComputers;
  private final UnsafeExternalRadixSorter sorter;
  ...
  public static UnsafeExternalRadixRowSorter create(
          StructType schema,
          Ordering<InternalRow> ordering,
          List<PrefixComparator> prefixComparators,
          List<UnsafeExternalRadixRowSorter.PrefixComputer> prefixComputers,
          long pageSizeBytes,
          boolean canUseRadixSort) throws IOException {
    // 空的列比较器
    Supplier<RecordComparator> recordComparatorSupplier =
            () -> new RowComparator(ordering, schema.length());
    return new UnsafeExternalRadixRowSorter(schema, recordComparatorSupplier, prefixComparators,
            prefixComputers, pageSizeBytes, canUseRadixSort);
  }

  private UnsafeExternalRadixRowSorter(
          StructType schema,
          Supplier<RecordComparator> recordComparatorSupplier,
          List<PrefixComparator> prefixComparators,
          List<UnsafeExternalRadixRowSorter.PrefixComputer> prefixComputers,
    ...
    this.schema = schema;
    this.prefixComputers = prefixComputers;
    final SparkEnv sparkEnv = SparkEnv.get();
    final TaskContext taskContext = TaskContext.get();
    sorter = UnsafeExternalRadixSorter.create(
 ...
 public void setTestSpillFrequency(int frequency) {
  ...
  public void insertRow(UnsafeRow row) throws IOException {
    final PrefixComputer.Prefix prefix1 = prefixComputers.get(0).computePrefix(row);
    final PrefixComputer.Prefix prefix2 = prefixComputers.get(1).computePrefix(row);
    sorter.insertRecord(
            row.getBaseObject(),
            row.getBaseOffset(),
            row.getSizeInBytes(),
