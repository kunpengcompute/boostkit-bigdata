 ...
 */
class SimpleCountTungstenAggIter(
                                   partIndex: Int,
                                   ...
                                   shouldDistinctCount: Boolean = false,
                                   DistinctCountValue: BigInt = -1)
  ...
  private def processInputs(fallbackStartsAt: (Int, Int)): Unit = {
    if (groupingExpressions.isEmpty) {
      ...
      if (shouldDistinctCount) {
        buffer.setLong(0, DistinctCountValue.toLong)
      } else {
        while (inputIter.hasNext) {
          val newInput = inputIter.next()
          buffer.setLong(0, buffer.getLong(0) + newInput.getLong(1))
        }
      }
    } else {
      var i = 0
