...
private[hive] object SparkSQLCLIDriver extends Logging {
  ...
  def reSetSparkArgument(array: Array[String]): Unit = {
    if (!NdpPluginEnableFlag.isEnable()) {
      return
    }
    ...
    for (i <- 0 until length - 1) {
      if (array(i) == "-f") {
        val sqlFile = new File(array(i + 1))
        if (!sqlFile.exists()) {
          return
        }
        ...
        sys.props("spark.locality.wait") = "8"
        sys.props("spark.locality.wait.legacyResetOnTaskLaunch") = "false"
        sys.props("spark.locality.wait.node") = "8"
        sys.props("spark.locality.wait.process") = "8"
        sys.props("spark.locality.wait.rack") = "8"
        sys.props("spark.shuffle.io.connectionTimeout") = "60s"
        sys.props("spark.network.timeout") = "120s"

        if (sql.contains("a,b") || sql.contains("join")) {
        ...
        if (!sys.props("spark.memory.offHeap.enabled").equalsIgnoreCase("true")) {
          return
        }
