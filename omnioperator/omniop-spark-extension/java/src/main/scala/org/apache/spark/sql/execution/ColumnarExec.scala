 ...
 */
case class ApplyColumnarRulesAndInsertTransitions(columnarRules: Seq[ColumnarRule])
  ...
  private def insertRowToColumnar(plan: SparkPlan): SparkPlan = {
    if (!plan.supportsColumnar) {
      ...
      RowToOmniColumnarExec(insertTransitions(plan))
  ...
  private def insertTransitions(plan: SparkPlan): SparkPlan = {
    if (plan.supportsColumnar) {
      ...
      if (conf.getConfString("spark.omni.sql.columnar.columnarToRow", "true").toBoolean) {
        OmniColumnarToRowExec(insertRowToColumnar(plan))
      } else {
