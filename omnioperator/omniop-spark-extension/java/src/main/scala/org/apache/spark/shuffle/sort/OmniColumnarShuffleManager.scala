...
class OmniColumnarShuffleManager(conf: SparkConf) extends ColumnarShuffleManager with Logging {

  import OmniColumnarShuffleManager._
  ...
  override def getReader[K, C](
    ...
    if (handle.isInstanceOf[ColumnarShuffleHandle[K, _]]) {
      new BlockStoreShuffleReader(
        ...
        context,
        metrics,
        serializerManager = bypassDecompressionSerializerManager,
...
private[spark] object OmniColumnarShuffleManager extends Logging {
  ...
  private def bypassDecompressionSerializerManager =
