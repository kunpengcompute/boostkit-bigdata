...
class ColumnShuffleSerializerSnappySuite extends SparkFunSuite with SharedSparkSession {
  ...
  test("columnar shuffle deserialize no null snappy compressed") {
    val input = getTestResourcePath("test-data/shuffle_spilled_mix_1batch_100rows_snappy")
    val serializer =
