...
class ColumnShuffleSerializerZlibSuite extends SparkFunSuite with SharedSparkSession {
  private var avgBatchNumRows: SQLMetric = _
  private var outputNumRows: SQLMetric = _

  override def sparkConf: SparkConf =
    super.sparkConf
      .setAppName("test ColumnarShuffleDeSerializer zlib compressed")
  ...
  test("columnar shuffle deserialize no null snappy compressed") {
    val input = getTestResourcePath("test-data/shuffle_spilled_mix_1batch_100rows_zlib")
    val serializer =
