...
class ColumnarWindowExecSuite extends ColumnarSparkPlanTest {
  ...
  test("validate columnar window exec happened") {
    ...
    assert(res.queryExecution.executedPlan.find(_.isInstanceOf[ColumnarWindowExec]).isDefined, s"ColumnarWindowExec not happened, executedPlan as follow: \n${res.queryExecution.executedPlan}")
  }
