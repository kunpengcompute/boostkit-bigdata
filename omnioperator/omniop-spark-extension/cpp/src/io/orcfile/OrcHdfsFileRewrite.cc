...
namespace orc {
  ...
  class HdfsFileInputStreamRewrite : public InputStream {
  private:
    std::string filename;
  ...
  public:
    HdfsFileInputStreamRewrite(std::string _filename) {
        std::vector<hdfs::Token*> tokens;
        HdfsFileInputStreamRewrite(_filename, tokens);
    }

    HdfsFileInputStreamRewrite(std::string _filename, std::vector<hdfs::Token*>& tokens) {
      filename = _filename ;
      ...
      if (!tokens.empty()) {
        for (auto input : tokens) {
            hdfs::Token token;
            token.setIdentifier(input->getIdentifier());
            token.setPassword(input->getPassword());
            token.setKind(input->getKind());
            token.setService(input->getService());
    ...
    ~HdfsFileInputStreamRewrite() override;
  };

  HdfsFileInputStreamRewrite::~HdfsFileInputStreamRewrite() {
  }

  std::unique_ptr<InputStream> readHdfsFileRewrite(const std::string& path, std::vector<hdfs::Token*>& tokens) {
    return std::unique_ptr<InputStream>(new HdfsFileInputStreamRewrite(path, tokens));
  }
}
