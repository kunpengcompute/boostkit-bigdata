...
public class BenchmarkHashAggregationOlkOperator
{
            ...
            extends AbstractOlkOperatorBenchmarkContext
    {
        ...
        private OperatorFactory createStreamingAggregationOperatorFactory()
        {
            return new StreamingAggregationOperatorFactory(0, new PlanNodeId("test"),
                    ImmutableList.of(VARCHAR), ImmutableList.of(VARCHAR), ImmutableList.of(0), AggregationNode.Step.SINGLE,
        ...
        private OperatorFactory createHashAggregationOperatorFactory()
        {
            ...
            return new HashAggregationOperatorFactory(0, new PlanNodeId("test"),
                    allTypes.get(sqlId), channels.get(sqlId),
                    ImmutableList.of(), AggregationNode.Step.SINGLE, false,
                    ImmutableList.of(),
                    Optional.of(allTypes.get(sqlId).size()), Optional.empty(), 100_000, Optional.of(new DataSize(16, MEGABYTE)), false,
                    succinctBytes(8), succinctBytes(Integer.MAX_VALUE), spillerFactory, joinCompiler, false);
        }
    ...
    public static void main(String[] args) throws RunnerException
    {
        Options options = new OptionsBuilder().verbosity(VerboseMode.NORMAL)
                .include(".*" + BenchmarkHashAggregationOlkOperator.class.getSimpleName() + ".*").build();
