...
public class BenchmarkHashJoinOmniOperators
{
            ...
            extends AbstractOperatorBenchmarkContext
    {
        ...
        private VecAllocator buildVecAllocator;
        ...
        protected List<Page> forkBuildPage(List<Page> pages)
        {
            ...
            return transferToOffHeapPages(buildVecAllocator, slicedPages);
        }

        @Override
        protected OperatorFactory createOperatorFactory()
        {
            HashBuilderOmniOperatorFactory hashBuilderOperatorFactory = createBuildOperatorFactory();
            LookupJoinOmniOperators.innerJoin(HASH_JOIN_OPERATOR_ID, TEST_PLAN_NODE_ID, lookupSourceFactoryManager,
                    getBuildTypes(), buildJoinChannels, buildHashChannel,
                    Optional.of(buildOutputChannels), OptionalInt.of(1), hashBuilderOperatorFactory);
        ...
        protected HashBuilderOmniOperatorFactory createBuildOperatorFactory()
        {
            ...
            return new HashBuilderOmniOperatorFactory(HASH_BUILD_OPERATOR_ID,
                    TEST_PLAN_NODE_ID, lookupSourceFactoryJoinBridgeManager, getBuildTypes(),
                    buildOutputChannels, buildJoinChannels, buildHashChannel, Optional.empty(), Optional.empty(),
                    ImmutableList.of(), 1);
        }
        ...
        {
            TaskContext testingTaskContext = createTaskContextBySizeInGigaByte(4);
            buildVecAllocator = VecAllocatorHelper.createTaskLevelAllocator(testingTaskContext);
            ...
            extends BuildContext
    {
        ...
        private VecAllocator probeVecAllocator;
        ...
        protected List<Page> forkPages(List<Page> pages)
        {
            ...
            return transferToOffHeapPages(probeVecAllocator, slicedPages);
        }
        ...
        {
            TaskContext testingTaskContext = createTaskContextBySizeInGigaByte(4);
            probeVecAllocator = VecAllocatorHelper.createTaskLevelAllocator(testingTaskContext);
            return testingTaskContext;
        }
        ...
        protected OperatorFactory createOperatorFactory()
        {
            ...
            OperatorFactory operatorFactory = LookupJoinOmniOperators.innerJoin(HASH_JOIN_OPERATOR_ID, TEST_PLAN_NODE_ID, lookupSourceFactoryManager,
                    getProbeTypes(), probeJoinChannels, probeHashChannel,
                    Optional.of(probeOutputChannels), OptionalInt.of(1), hashBuilderOperatorFactory);
    ...
    public static void main(String[] args) throws RunnerException
    {
        Options options = new OptionsBuilder().verbosity(VerboseMode.NORMAL)
                .include(".*" + BenchmarkHashJoinOmniOperators.class.getSimpleName() + ".*").build();
